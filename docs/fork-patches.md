...
| `core/deltachat-jsonrpc/src/api.rs` | Four more imports and a second contiguous block of thin methods at the end of `impl CommandApi`, covering tags, gating and the trash. Also `outgoing` and `to` computed inside `get_message_rows`, and a sixth `html` parameter on `send_email`. | Same constraint as the first block: `yerpc`'s `#[rpc]` applies to one `impl`. Re-place the block rather than replaying the diff. **`send_email`'s arity is load-bearing**: `yerpc` compares positional params with `!=`, so a caller passing five arguments to a six-parameter method gets `invalid params`, not a `None`. Every caller moves together or none does. | [0017](adr/0017-system-tags.md), [0018](adr/0018-contact-gating.md), [0019](adr/0019-recoverable-ephemeral-expiry.md), [0025](adr/0025-composed-html.md) |
...
| `core/src/sql/migrations.rs` | Migration 175: `CREATE TABLE msg_send_options`. | The composer's per-message padlock, whether it placed the signature in the body, and the signature it split off that body. Not `msgs.param`: `chat::send_msg` strips `ForcePlaintext` and `GuaranteeE2ee` from a message saved as a draft, which every composed message is. No row means the defaults. | [0032](adr/0032-composer-send-options.md) |
